...
interface EditTuyenDuongModalProps {
  editData: any | null;
        ...
        >
          ...
          <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-sky-600 text-white rounded-xl shadow-md shadow-sky-100">
                <Map className="w-5 h-5" />
              </div>
              <div>
                <h2 className="text-base font-black text-slate-800 uppercase tracking-wide">Cập nhật Tuyến Đường</h2>
          ...
          <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-5">
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all font-semibold"
                value={editData.ten || ''}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all"
                  value={editData.loai || ''}
            ...
            {editData.coordinates && (
              <div className="p-4 bg-sky-50/50 rounded-2xl border border-sky-100/50 flex flex-col gap-3">
                <div className="text-[10px] text-sky-850 font-mono">
                  <span className="font-bold">Đặc điểm GIS:</span> Tuyến gồm {editData.coordinates.length} tọa độ điểm LineString.
                </div>
                {onEditCoordinates && (
                  <button
                    type="button"
                    onClick={() => onEditCoordinates(editData)}
